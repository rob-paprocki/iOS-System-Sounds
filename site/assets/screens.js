...
export function renderHome(host) {
  ...
  host.innerHTML = `
    <section class="lede">
      <h2>Every sound Apple shipped, ${esc(span)}</h2>
      <p>Each release of iOS carries a few hundred audio files: the alert you
        half-remember, the lock click that changed in 2013, a thousand stems for
        Memories soundtracks nobody hears separately. This is all of them, pulled
        from the root filesystem of ${data.versions.length} builds and
        de-duplicated by audio identity, so every entry knows exactly which
        releases it shipped in.</p>
      <dl class="tally">
        <div><dt>Distinct sounds</dt><dd>${fmtInt(c.total)}</dd></div>
        <div><dt>Still in iOS</dt><dd>${fmtInt(c.present)}</dd></div>
    ...
    <section class="home-shelves" id="home-shelves">
      ...
      <p class="section-note">Press play. Nothing here plays on its own.</p>
    </section>
...
export function renderContents(host) {
  ...
  host.innerHTML = `
    ...
    <section>
      <h3>Categories</h3>
      <table class="data-table">
        <thead><tr><th scope="col">Category</th><th scope="col" class="num">Still in iOS</th>
