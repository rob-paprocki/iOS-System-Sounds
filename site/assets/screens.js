...
export function renderHome(host) {
  ...
  host.innerHTML = `
    ...
    <section class="lede lede-split">
      <div class="lede-text">
        ...
        <p>Each release carries a few hundred audio files: the alert you
          ...
          builds. Identical audio collapses into one entry, so each one lists every
          release it appeared in.</p>
        <p class="cta-row">
    ...
    <section class="built">
      ...
      <ol class="steps">
        <li>Download one IPSW per iOS release line, plus every iPhone launch build.</li>
        <li>Mount the root filesystem and walk all of it for audio. It hides in
          <code>UISounds</code>, framework bundles, accessibility bundles and ringtones.</li>
        <li>Compare every build against the rest by four escalating identity tests, so a
          file that never changed becomes one entry covering its whole run.</li>
        <li>Sort what still ships into <code>Current/</code> and the rest into
          <code>Removed/</code>, keeping the bulk spoken-word assets aside so they do
          not swamp the system sounds.</li>
      </ol>
    ...
    <section class="download-shape">
      ...
      <p>Apple's original file, byte for byte, in whatever format Apple shipped it in:
        <code>.caf</code>, <code>.aiff</code>, <code>.wav</code> or <code>.m4a</code>.
        The pipeline never re-encodes them. The AAC previews exist only so the browser
        has something it can play, because <code>.caf</code> and <code>.aiff</code> do
        not decode outside Safari.</p>
      ...
      <tbody>
        ${top.map(r => `
          <tr>
            ...
            <td class="num mono mark-add">${r.a ? '+' + fmtInt(r.a) : '0'}</td>
            <td class="num mono mark-del">${r.d ? '−' + fmtInt(r.d) : '0'}</td>
          </tr>`).join('')}
      </tbody>
    </table>
  ...
  host.innerHTML = `
    <section class="lede">
      ...
      <p>${fmtInt(c.total)} distinct sounds across ${data.versions.length} builds,
        ...
        merged or renamed to tidy the list up.</p>
    </section>
...
export function renderMemories(host) {
  ...
  host.innerHTML = `
    <section class="lede">
      ...
      <p>${fmtInt(total)} stems across ${names.length} soundtracks. That is more than a
        third of this whole collection, and the part of it nobody ever hears named.
        Pick a soundtrack, choose one stem per role, and play the sequence back.</p>
    </section>
